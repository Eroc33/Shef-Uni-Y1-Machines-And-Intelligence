...
public class RunLatticeSearch {
  ...
  public static void main(String[] arg) {
    if(arg.length!=2){
      System.out.println("Usage: java RunLatticeSearch TEST STRATEGY");
      System.out.println("where TEST is `example` or `tv1`");
      System.out.println("where STRATEGY is `depthFirst` or `breadthFirst` or `branchAndBound`");
      return;
    }
    String test = arg[0];
    String strat = arg[1];
    if(test.equalsIgnoreCase("example")){
      exampleTest(strat);
    }else if(test.equalsIgnoreCase("tv1")){
      tvTest("latttv1.txt",strat);
    }else if(test.equalsIgnoreCase("tv2")){
      tvTest("latttv2.txt",strat);
    }else{
      System.out.println("Usage: javaRunLatticeSearch TEST STRATEGY");
      System.out.println("where TEST is `example` or `tv1`");
      System.out.println("where STRATEGY is `depthFirst` or `breadthFirst` or `branchAndBound`");
    }
  }

  public static void tvTest(String filename,String strat){
...
// Language Model costMatrix for control vocab
    ...
    runLattice(latt,tVocab,tCosts,strat);
  }

  public static void exampleTest(String strat){
    // create a WordLattice
    ...
    runLattice(latt,vocab,costs,strat);
  }

  public static void runLattice(WordLattice latt,  String [] vocab, int [][] costs,String strat) {
    ...
    SearchState initState = (SearchState) new LatticeState(new WordH("*start*",0,0,0),0,latt);

    // then you can run the search:

    String res_bb = lsearch.runSearch(initState, "breadthFirst");
